...
namespace New_LerayBookingSystem.Controllers
{
        ...
        IEmailService emailService) : ControllerBase // <-- ADDED IEmailService
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterDto model)
        {
            ...
            // ✅ Validate and prevent duplicate phone number (if provided)
            if (!string.IsNullOrWhiteSpace(model.PhoneNumber))
            {
                var phPhoneRegex = new Regex(@"^(?:\+639|09)\d{9}$");
                if (!phPhoneRegex.IsMatch(model.PhoneNumber))
                    return BadRequest(new { message = "Invalid PH phone number format." });

                // FIX: Use FirstOrDefaultAsync for async DB query
